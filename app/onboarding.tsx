...
export default function OnboardingScreen() {
  ...
  const { state, saveProfile, useDemoData: loadDemoData } = useStockbook();
  const editing = mode === "edit" && !!state.profile;
  const profile = state.profile;
  const [step, setStep] = useState(editing ? slides.length : 0);
  const [businessName, setBusinessName] = useState(profile?.businessName ?? "");
  const [businessType, setBusinessType] = useState(profile?.businessType ?? "Kiosk");
  const [ownerName, setOwnerName] = useState(profile?.ownerName ?? "");
  const [openingBalance, setOpeningBalance] = useState(profile ? String(profile.defaultOpeningBalance) : "");
  const [mpesaNumber, setMpesaNumber] = useState(profile?.mpesaNumber ?? "");
  const [language, setLanguage] = useState<"en" | "sw">(profile?.language ?? "en");
  const slide = slides[step];
  const goHome = () => { if (editing) router.back(); else router.replace("/"); };
  const handleSave = () => {
  ...
  const startDemo = () => { loadDemoData(); goHome(); };
